...
//     return context;
// };
// woofwalks_front/src/contexts/AuthContext.js
import axios from 'axios';
import { createContext, useCallback, useContext, useEffect, useState } from 'react';
...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = useCallback(async () => {
        try {
            ...
            const apiUrl = process.env.NODE_ENV === 'production' ? '/api' : process.env.REACT_APP_API_URL;
            ...
            async err => {
                ...
                if (err.response?.status === 401 && !originalRequest.url.endsWith('/api/token/refresh') && !isRefreshing) {
                    ...
                    try {
                        console.log('Trying to refresh token...');
                        const apiUrl = process.env.NODE_ENV === 'production' ? '/api' : process.env.REACT_APP_API_URL;
                        await axios.post(`${apiUrl}/api/token/refresh`, {}, { withCredentials: true });
                        isRefreshing = false;
                        console.log('Token refreshed, retrying original request...');
                        return axios({ ...originalRequest, withCredentials: true });
                    } catch (refreshErr) {
